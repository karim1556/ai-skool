...
export default function EditBatchPage() {
  ...
  // Load trainers when school changes
  useEffect(() => {
  ...
  // Prefill existing batch
  useEffect(() => {
    const loadBatch = async () => {
      if (!id) return
      try {
        const res = await fetch(`/api/batches?id=${id}`, { cache: 'no-store' })
        const rows = await res.json()
        const batch = Array.isArray(rows) ? rows[0] : rows
        if (!batch) return
        setName(batch.name || "")
        setSchoolId(batch.school_id || "")
        setCourseId(batch.course_id || "")
        setStatus(batch.status || "verified")
        setMaxStudents(batch.max_students ? String(batch.max_students) : "")
        setStartDate(batch.start_date || "")
        setEndDate(batch.end_date || "")
        setSchedule(batch.schedule || "")
        setDescription(batch.description || "")
        const trainerIds = (batch.trainer_ids as string | null)?.split(',').filter(Boolean) || []
        const studentIds = (batch.student_ids as string | null)?.split(',').filter(Boolean) || []
        setSelectedTrainerIds(new Set(trainerIds))
  ...
  const handleComplete = async () => {
    try {
      if (!id) {
        toast({ title: "Missing batch id", variant: "destructive" })
        return
      }
      if (!name) {
      ...
      if (!res.ok) {
        const msg = data?.error || data?.message || (await res.text().catch(() => "Failed to update batch"))
        throw new Error(`${res.status} ${res.statusText}: ${msg}`)
      }
      toast({ title: "Batch updated" })
      router.push("/admin/batches")
    } catch (e: any) {
      toast({ title: "Error", description: e?.message || String(e) || "Failed to update batch", variant: "destructive" })
    }
  }
